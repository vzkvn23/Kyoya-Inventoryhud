$(document).ready(function () {
	...
	function setupImpoundForm(data) {
		...
		if (data.samsat) {
			$('#samsat').val(data.samsat).prop('disabled', true);
		}
	...
	function validateImpoundForm () {
		...
		if(String(weeks).length < 1 || parseInt(weeks) < 0 || parseInt(weeks) > rules.maxWeeks || String(days).length < 1 || parseInt(days) < reason.minDays || parseInt(days) > rules.maxDays || String(hours).length < 1 || parseInt(hours) < 0 || parseInt(hours) > rules.maxHours) {
			errors.append(`<small>&#9679; Pengisian Waktu dalam Seminggu dapat diisi 0 atau maksimal ${rules.maxWeeks}, hari juga dapat diisi 0 atau maksimal ${rules.maxDays} dan jam juga dapat diisi 0 atau maksimal ${rules.maxHours}.</small>`);
			success = false;
		}

		if(fee.isNaN || String(fee).length < 1 || parseInt(fee) < rules.minFee || parseInt(fee) > rules.maxFee) {
			errors.append(`<small>&#9679; Denda jangan kurang dari ${rules.minFee} atau lebih dari ${rules.maxFee}</small>`);
			success = false;
		}

		if(reason.length < rules.minReasonLength || reason.length > 10000) {
			errors.append(`<small>&#9679; Alasan Penyitaan tidak dapat kurang dari ${rules.minReasonLength} Kata.</small>`);
			success = false;
		}
	...
	function setupRetrievalForm(data) {
		...
		if(rData.vehicles[0].hold_o) {
			$(holdBy).text(`Kendaraan Ini Harus Dapat izin dari Polisi`);
		} else if (rData.vehicles[0].hold_m) {
			$(holdBy).text(`Kendaraan Ini Harus Dapat izin dari petugas Samsat`);
		} else {
			$(holdBy).text(``);
		}
	...
	function setupAdminTerminal (data) {
		...
		for(var i = 0; i < data.vehicles.length; i++) {
			...
			if((data.job.name == "police" && data.vehicles[i].hold_o) || (data.job.name == "samsat" && data.vehicles[i].hold_m)) {
				var button = `<td>
					<button class="btn info mr" id="info${i}">Info</button>
