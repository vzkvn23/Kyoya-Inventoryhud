...
----------------------------------------------------------------------------------------------------
RegisterNetEvent("HRP:ESX:impundmenueuy")
AddEventHandler("HRP:ESX:impundmenueuy", function()
...
function ShowImpoundMenu (action)
	...
	if (vehicle ~= nil) then
		...
		if (_XPlayer.job.name == 'samsat') then
...
Citizen.CreateThread(function ()
	...
	while true do
		...
		if(_DependenciesLoaded) then
			...
			elseif IsAnyVehicleNearPoint(PlayerPedCoords.x, PlayerPedCoords.y, PlayerPedCoords.z, 3.0) then

				inZone = true;

				if (_CurrentAction ~= "store" and (_XPlayer.job.name == "police" or _XPlayer.job.name == "samsat")) then
				...
				for i, location in ipairs(_Impound.AdminTerminalLocations) do
					...
					PlayerPedCoords.x, PlayerPedCoords.y, PlayerPedCoords.z, false) < 3) then
						...
						if (_CurrentAction ~= "admin" and (_XPlayer.job.name == "police" or _XPlayer.job.name == "samsat")) then
